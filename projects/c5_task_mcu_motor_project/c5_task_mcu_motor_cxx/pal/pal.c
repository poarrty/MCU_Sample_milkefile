...
void task_motor_set_info(const void *msgin) {
    ...
    if (root != NULL) {
        microros_proxy.message_get.name        = cJSON_GetObjectItemCaseSensitive(root, "name")->valuestring;
        microros_proxy.message_get.value.speed = (int) ((cJSON_GetObjectItemCaseSensitive(root, "value")->valuedouble) * 100.0);
...
SHELL_EXPORT_CMD(SHELL_CMD_PERMISSION(0) | SHELL_CMD_TYPE(SHELL_TYPE_CMD_FUNC), cpp_test, cpp_test, cpp_test);