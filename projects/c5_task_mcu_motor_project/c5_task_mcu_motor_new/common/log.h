...
#define log_printf(format, ...)                                \
    do {                                                       \
        ...
        shellWriteEndLine(&shell, log_buff, strlen(log_buff)); \
    } while (0)

#define LOG_INFO(format, ...)                                                                                                 \
    do {                                                                                                                      \
        if (log_level >= LOG_LEVEL_INFO) {                                                                                    \
            log_printf(INFO_COLOR "[log_i]:%s[%04d]: " format "\r\n" COLOR_END, filename(__FILE__), __LINE__, ##__VA_ARGS__); \
        }                                                                                                                     \
    } while (0)
...
#define LOG_DEBUG_ARRAY(array, num)                                                                            \
    do {                                                                                                       \
        if (log_level >= LOG_LEVEL_DEBUG_ARRAY) {                                                              \
            uint32_t i;                                                                                        \
            uint8_t *a = array;                                                                                \
            sprintf(log_buff, DEBUG_ARRAY_COLOR "[log_a]:%s[%04d]: " COLOR_END, filename(__FILE__), __LINE__); \
            for (i = 0; i < num; i++) {                                                                        \
                sprintf(temp_buff, "%02x ", a[i]);                                                             \
