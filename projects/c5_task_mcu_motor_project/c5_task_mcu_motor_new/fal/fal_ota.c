...
#define APP_KEY "9af26061496ff3e031c79974784382bcc27ddd27"
...
void fal_ota_discovered(const void *msgin) {
    log_d("OTA discoverd");
    strcpy(kv_firmware_info.version, SOFTWARE_VERSION);
    log_i("kv_firmware_info.version :%s", kv_firmware_info.version);

    firmware_info.header.stamp = kv_firmware_info.stamp;

    firmware_info.priority = 5;
...
void fal_get_updating_info(const void *req, void *res) {
    // chassis_interfaces__srv__GetUpdatingInfo_Request * req_in = (chassis_interfaces__srv__GetUpdatingInfo_Request *) req;
