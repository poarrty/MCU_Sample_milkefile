...
static publisher     g_motor_heartbeat;
std_msgs__msg__UInt8 motor_heartbeat;
...
void message_publish(ppublisher ptpublisher) {
    ...
    if (ptpublisher->publish_ret == RCL_RET_OK) {
        log_d("%s pub ok", ptpublisher->topic_name);
    } else {
...
bool MicroRosFreeResource(bool *freeFlag) {
    while (topic_remain_count) {
        osDelay(500);
        log_d("wait remain topic pub fail");
    }
...
bool MicroRosBuildResource() {
    ...
    ret = rclc_node_init_default(&g_node, "kavabotPile", "", &g_support) == RCL_RET_OK ? true : false;
    ...
    /* timer *****************************************************************/
    // RCCHECK(rclc_timer_init_default(&ipc2uros_timer, &g_support, RCL_MS_TO_NS(1), ipc_timer_callback));

    /* executor **************************************************************/
    // Create a executor
    log_d("rclc_executor_init");

    if (rclc_executor_init(&g_executor, &g_support.context, subscrption_register_count + service_register_count, &g_allocator) != RCL_RET_OK) {
...
bool IsSyncTime() {
    ...
    sync_ts                     = ret ? osKernelGetTickCount() + 60 * 1000 : sync_ts;
    return ret;
}
...
SyncStatus MicroRosSyncTime() {
    ...
    bsp_rtc_set(time_utc->tm_year + 1900, time_utc->tm_mon + 1, time_utc->tm_mday, time_utc->tm_hour, time_utc->tm_min, time_utc->tm_sec,
...
void StartDefaultTask(void *argument) {
    ...
    publisher_init(&g_motor_heartbeat, ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, UInt8), "/c5_task_mcu_motor_heartbeat", &motor_heartbeat,
    ...
    while (1) {
        ...
        if (syncStatus != SYNC_STATUS_SUCCEED && !ping_ok) {
        ...
        if (IsHeartBeat()) {
            if (g_motor_heartbeat.publish_ret != RCL_RET_OK) {
            ...
            message_publish(&g_motor_heartbeat);
        }
